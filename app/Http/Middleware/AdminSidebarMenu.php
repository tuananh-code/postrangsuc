        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                    ...
                    function ($sub) use ($common_settings) {
                        if (auth()->user()->can('buysell.view') || auth()->user()->can('view_own_purchase')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'buysell' && request()->segment(2) == null]
                            );
                        }
                        if (auth()->user()->can('buysell.create')) {
                            $sub->url(
                                action('BuySellController@create'),
                                __('Giao dịch hàng sỉ'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'buysell' && request()->segment(2) == 'create']
                            );
                        }
                        if (auth()->user()->can('buysell.create')) {
                            $sub->url(
                                action('BuySellPosController@create'),
                                __('Giao dịch hàng lẻ'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'posbuysell' && request()->segment(2) == 'create']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-arrow-circle-down', 'id' => 'tour_step9']
                )->order(25);
            }
            ...
            $menu->dropdown(
                ...
                function ($sub) use ($enabled_modules) {
                    ...
                    $sub->url(
                        action('ReportFormController@getReportFormPnj'),
                        'Giá vàng PNJ',
                        // ['icon' => 'fa fas fa-cogs', 'active' => request()->segment(1) == 'business', 'id' => "tour_step2"]
                    );
                    $sub->url(
                        action('ReportFormController@getReportFormSjc'),
                        'Giá vàng SJC',
                        // ['icon' => 'fa fas fa-map-marker', 'active' => request()->segment(1) == 'business-location']
                    );
                    $sub->url(
                        action('ReportFormController@getReportFormDoji'),
                        'Giá vàng DOJI',
                        // ['icon' => 'fa fas fa-map-marker', 'active' => request()->segment(1) == 'business-location']
                    );
                },
                ['icon' => 'fa fas fa-pager', 'id' => 'tour_step10']
            )->order(85);
            ...
            $menu->dropdown(
                ...
                function ($sub) use ($enabled_modules) {
                    // if (auth()->user()->can('business_settings.access')) {
                    $sub->url(
                        action('TemplateController@getPnjTemplate'),
                        'PNJ Website',
                        // ['icon' => 'fa fas fa-cogs', 'active' => request()->segment(1) == 'business', 'id' => "tour_step2"]
                    );
                    $sub->url(
                        action('TemplateController@getSjcTemplate'),
                        'SJC Website',
                        // ['icon' => 'fa fas fa-map-marker', 'active' => request()->segment(1) == 'business-location']
                    );
                    
                },
                ['icon' => 'fa fas fa-globe-asia', 'id' => 'tour_step11']
            )->order(85);
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('invoice_settings.access')) {
                            ...
                            // $sub->url(
                            //     action('QrcodeController@index'),
                            // 'Cài đặt mã QR',
                            //     ['icon' => 'fa fas fa-barcode', 'active' => request()->segment(1) == 'QR-code']
                            // );
