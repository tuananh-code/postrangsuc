...
class ContactUtil extends Util
{
    ...
    public function updateContact($input, $id, $business_id, $edit = false, $transaction = null)
    {   
        $count = 0;
        //Check Contact id  
        ...
        if ($count == 0) {
            ...
            if(!isset($input['location_id'])) {
                ...
                $balance = isset($input['wages_due']) ? $input['wages_due'] : 0;
                $balance_gold = isset($input['gold_due']) ? $input['gold_due'] : 0;
                if ($edit) {
                    
                    $balance_old = $transaction->last_debt_wages;
                    $balance_gold_old = $transaction->last_debt_gold;
                    $balance_new = $balance;
                    $balance_gold_new = $balance_gold;
                    $current_debt = $contact->balance;
                    $current_debt_gold = $contact->debt_gold;
                    $balance = ($balance_new - $balance_old) + $current_debt;
                    $balance_gold = ($balance_gold_new - $balance_gold_old) + $current_debt_gold;
                    
                }
                $contact->setAttribute('balance', floatval(preg_replace('/[^0-9.]/', '', $balance)));
                $contact->setAttribute('debt_gold', floatval(preg_replace('/[^0-9.]/', '', $balance_gold)));
