...
class TransactionUtil extends Util
{
    ...
    {   
        ...
        $input['total_money'] = !empty($input['total_money']) ? $input['total_money'] : '0';
        $wages = str_replace(',', '', $input['wages']);

        if(isset($input['sales_form']) && $input['sales_form']== 'retail') {
            $final_total = $uf_data ? $this->num_uf($input['final_total']) : $input['final_total'];
        } else {
            $final_total = $uf_data ? $this->num_uf($input['total_money']) : $input['total_money'];
        }
        $gold_price = str_replace(',', '', $input['pri']);
        $transaction = Transaction::create([
            ...
            'ref_no' => '',
            'total_before_tax' => !empty($invoice_total['total_before_tax']) ? $invoice_total['total_before_tax'] : 0,
            ...
            'tax_amount' => !empty($invoice_total['tax']) ? $invoice_total['tax'] : 0,
            ...
            'standard' => !empty($input['standard']) ? $input['standard'] : 0,
            'final_weight' => !empty($input['final_weight']) ? $input['final_weight'] : 0,
            'gold_price' => !empty($gold_price) ? $gold_price : 0,
            'gold_paid' => ($input['gold_want_to_pay'] != 'NaN') && !empty($input['gold_want_to_pay']) ? $input['gold_want_to_pay'] : 0,
            'wages_paid' => !empty($wages) ? $wages : 0,
            'wages_debt_old' => !empty($input['advance_balance']) ? $input['advance_balance'] : 0,
            'gold_debt_old' => !empty($input['advance_balance_gold']) ? $input['advance_balance_gold'] : 0,
            'last_debt_gold' => !empty($input['balance_due']) ? $input['balance_due'] : 0,
            'last_debt_wages' => !empty($input['wages_due']) ? $input['wages_due'] : 0,
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        $update_date = [
            ...
            'standard' => !empty($input['standard']) ? $input['standard'] : null,
            'final_weight' => !empty($input['final_weight']) ? $input['final_weight'] : 0,
            'gold_price' => !empty($gold_price) ? $gold_price : 0,
            'gold_paid' => !empty($input['gold_want_to_pay']) ? $input['gold_want_to_pay'] : 0,
            'wages_paid' => !empty($wages) ? $wages : 0,
            'wages_debt_old' => !empty($input['advance_balance']) ? $input['advance_balance'] : 0,
            'gold_debt_old' => !empty($input['advance_balance_gold']) ? $input['advance_balance_gold'] : 0,
            'last_debt_gold' => !empty($input['balance_due']) ? $input['balance_due'] : 0,
     ...
     */
    public function createOrUpdateSellLines($transaction, $products, $location_id, $buy = null, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
            ...
            foreach ($products as $product) {
                ...
                if (!empty($product['transaction_sell_lines_id'])) {
                    ...
                    $line = [
                        ...
                        'total_weight' => !empty($product['weight']) ? $product['weight'] : 0,
                        'weight_seed' => !empty($product['seed_weight']) ? $product['seed_weight'] : 0,
                        'golden_age' => !empty($product['golden_age']) ? $product['golden_age'] : 0,
        ...
        if (!empty($buy)) {
            foreach ($buy as $buy) {
                if (!empty($buy['buy_id_edit'])) {
                    $edit_id_temp_b = $this->editBuyLine($buy);

                    $edit_idb = array_merge($edit_idb, $edit_id_temp_b);
                    //update or create modifiers for existing sell lines
                } else {
                    $linebuy = [
                        'sectors' => !empty($buy['sectors']) ? $buy['sectors'] : null,
                        'total_weight' => !empty($buy['weight']) ? $buy['weight'] : 0.0,
                        'weight_seed' => !empty($buy['seed_weight']) ? $buy['seed_weight'] : 00,
                        'golden_age' => !empty($buy['golden_age']) ? $buy['golden_age'] : 0.0,
                        'price' => isset($buy['price']) && !empty($buy['price']) ? $buy['price'] : 0.0
                    ];

                    $lines_formatted_buy[] = new TransactionBuyLine($linebuy);
                }
            }
        }
        ...
        if (!empty($edit_idb)) {
            $deleted_lineb = TransactionBuyLine::where('transaction_id', $transaction->id)
                ->whereNotIn('id', $edit_idb)
                ->select('id')->get()->toArray();
            $this->deleteBuyLines($deleted_lineb);
        }
        ...
        if (!empty($lines_formatted_buy)) {
            $transaction->buy_lines()->saveMany($lines_formatted_buy);
        }
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1)
    {
        ...
        $sell_line->fill(['product_id' => $product['product_id'],
            ...
            'total_weight' => !empty($product['weight']) ? $product['weight'] : null,
            'weight_seed' => !empty($product['seed_weight']) ? $product['seed_weight'] : null,
            'golden_age' => !empty($product['golden_age']) ? $product['golden_age'] : null
        ]);
        $sell_line->save();
     ...
     * @return boolean
     */
    public function editBuyLine($buy)
    {
        
        //Get the old order quantity
        $buy_line = TransactionBuyLine::find($buy['buy_id_edit']);
        $edit_idb[] = $buy['buy_id_edit'];
        //Update sell lines.
        $buy_line->fill([
            'sectors' => !empty($buy['sectors']) ? $buy['sectors'] : null,
            'total_weight' => !empty($buy['weight']) ? $buy['weight'] : null,
            'weight_seed' => !empty($buy['seed_weight']) ? $buy['seed_weight'] : null,
            'golden_age' => !empty($buy['golden_age']) ? $buy['golden_age'] : null,
        ]);
        $buy_line->save();
        return $edit_idb;
    }
     ...
     * @return boolean
     */
    public function deleteBuyLines($transaction_line_idb)
    {
        if (!empty($transaction_line_idb)) {
            //unset so_line_id if set
            TransactionBuyLine::whereIn('id', $transaction_line_idb)
                ->delete();
        }
    }
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                $payment_amount = $uf_data ? $this->num_uf(str_replace('₫', '', $payment['amount'])) : $payment['amount'];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $total_amount = intval(($transaction->gold_paid * $transaction->gold_price) / 100) + $transaction->wages_paid;
        $words  = $this->convert_number_to_words($total_amount);
        $user = auth()->user();
        ...
        $output = [
            ...
            'standard' => isset($transaction->standard) ? $transaction->standard : '',
            'final_weight' => isset($transaction->final_weight) ? $transaction->final_weight : '',
            'gold_price' => isset($transaction->gold_price) ? $transaction->gold_price : '',
            'gold_paid' => isset($transaction->gold_paid) ? $transaction->gold_paid : '',
            'wages_paid' => isset($transaction->wages_paid) ? $transaction->wages_paid : '',
            'wages_debt_old' => isset($transaction->wages_debt_old) ? $transaction->wages_debt_old : '',
            'gold_debt_old' => isset($transaction->gold_debt_old) ? $transaction->gold_debt_old : '',
            'last_debt_gold' => isset($transaction->last_debt_gold) ? $transaction->last_debt_gold : '',
            'last_debt_wages' => isset($transaction->last_debt_wages) ? $transaction->last_debt_wages : '',
            'bankName' => isset($bank_details->bank_name) ? $bank_details->bank_name : '',
            'accountNumber' => isset($bank_details->account_number) ? $bank_details->account_number : '',
            'accountHolderName' => isset($bank_details->account_holder_name) ? str_replace(' ', '%20', $bank_details->account_holder_name) : '',
            'bankCode' => isset($bank_details->bank_code) ? $bank_details->bank_code : '',
            'branch' => isset($bank_details->branch) ? $bank_details->branch : '',
            'taxPayerId' => isset($bank_details->tax_payer_id) ? $bank_details->tax_payer_id : '',
            'words' => ucfirst($words) ? ucfirst($words) : '',
        ];
        ...
        if ($il->show_customer == 1) {
            ...
            $output['balance'] = !empty($customer->balance) ? $customer->balance: 0;
            $output['debt_gold'] = !empty($customer->debt_gold) ? $customer->debt_gold : 0;
        ...
        $output['lines'] = [];
        $total_exempt = 0;
        if ($transaction_type == 'buysell' || $transaction_type == 'buy') {
            ...
            $lines_buy = $transaction->buy_lines()->get();
            ...
            $details_buy = [];
            foreach ($lines_buy as $item) {
                $details_lines = [
                    'id' => $item->id,
                    'transaction_id' => $item->transaction_id,
                    'sectors' => $item->sectors,
                    'total_weight' => $item->total_weight,
                    'weight_seed' => $item->weight_seed,
                    'golden_age' => $item->golden_age,
                    'price' => $item->price,
                ];
                $details_buy[] = $details_lines;
            }
            
            $output['lines'] = $details['lines'];
            $output['lines_buy'] = $details_buy;
            $output['taxes'] = [];
            $total_quantity = 0;
        ...
        switch (true) {
            ...
            case $number < 1000:
                ...
                if ($remainder) {
                    if($remainder < 10 && $remainder != 0) {
                        $string .= $conjunction . 'lẻ'. $conjunction . $this->convert_number_to_words($remainder);
                    } else {
            ...
            default:
                ...
                if ($remainder) {
                    ...
                    $string .=
                     $this->convert_number_to_words($remainder);
                }
    ...
    {   
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'name' => $product->name,
                'total_weight' => !empty($line->total_weight) ? $line->total_weight : 0,
                'seed_weight' => !empty($line->weight_seed) ? $line->weight_seed : 0,
                'golden_age' => !empty($line->golden_age) ? $line->golden_age : 0,
                'price' => !empty($line->price) ? $line->price : 0,
    ...
    public function getBuySellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->where('transactions.type', 'buysell')
            ->where('transactions.status', 'final')
            ->select(
                'transactions.id',
                'final_total',
    ...
    */
    ...
    public function getTransactionTotals(
        ...
        if (in_array('buysell', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='buysell' AND transactions.status='final', IF(discount_type = 'percentage', COALESCE(discount_amount, 0)*total_before_tax/100, COALESCE(discount_amount, 0)), 0)) as total_buysell_discount"),
                DB::raw("SUM(IF(transactions.type='buysell' AND transactions.status='final', rp_redeemed_amount, 0)) as total_buysell_reward_amount"),
                DB::raw("SUM(IF(transactions.type='buysell' AND transactions.status='final', round_off_amount, 0)) as total_buysell_round_off")
            );
        }
        ...
        if (in_array('buysell', $transaction_types)) {
            $output['total_buysell_discount'] =
            !empty($transaction_totals->total_buysell_discount) ?
                $transaction_totals->total_buysell_discount : 0;

            $output['total_buysell_reward_amount'] =
            !empty($transaction_totals->total_buysell_reward_amount) ?
                $transaction_totals->total_buysell_reward_amount : 0;

            $output['total_buysell_round_off'] =
            !empty($transaction_totals->total_buysell_round_off) ?
                $transaction_totals->total_buysell_round_off : 0;
        }
        return $output;
    }
    ...
    */
     ...
     * @return object
     */
    public function getListBuySell($business_id, $sale_type = 'sell')
    {   
        // ->leftJoin('transaction_payments as tp', 'transactions.id', '=', 'tp.transaction_id')
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
        ...
        ->leftJoin('transaction_sell_lines as tsl', function ($join) {
            ...
            'transactions.final_weight',
            'transactions.gold_price',
            'transactions.gold_paid',
            'transactions.wages_paid',
            'transactions.wages_debt_old',
            'transactions.gold_debt_old',
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        //Get Sell details
        $buysell_details = $this->getBuySellTotals(
            $business_id,
            $start_date,
            $end_date,
            $location_id,
        ...
        $transaction_types = [
            'purchase_return', 'sell_return', 'expense', 'stock_adjustment', 'sell_transfer', 'purchase', 'sell', 'buysell'
        ];
        ...
        $data['total_buysell_shipping_charge'] = !empty($buysell_details['total_shipping_charges']) ? $buysell_details['total_shipping_charges'] : 0;
        //Shipping
        ...
        $total_buysell_discount = $transaction_totals['total_buysell_discount'];
        ...
        $data['total_buysell'] = !empty($buysell_details['total_sell_exc_tax']) ? $buysell_details['total_sell_exc_tax'] : 0;
        $data['total_sell_discount'] = !empty($total_sell_discount) ? $total_sell_discount : 0;
        $data['total_buysell_discount'] = !empty($total_buysell_discount) ? $total_buysell_discount : 0;
