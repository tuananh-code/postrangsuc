    ...
    public function getInvoiceUrl($transaction_id, $business_id)
    {
        ...
        if (empty($transaction->invoice_token)) {
            ...
            if ($transaction->is_quotation == 1) {
                return route('show_quote_buysell', ['token' => $transaction->invoice_token]);
            }

            return route('show_invoice_buysell', ['token' => $transaction->invoice_token]);
            // if ($transaction->is_quotation == 1) {
            //     return route('show_quote_sell', ['token' => $transaction->invoice_token]);
            // }

            // return route('show_invoice_sell', ['token' => $transaction->invoice_token]);
