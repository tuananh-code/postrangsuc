<script type="text/javascript">
    $(document).ready(function() {
        ...
        function initializeTaxonomyDataTable() {
            ...
            if ($('#category_table').length) {
                    ...
                    columns: [{
                        ...
                        @endif {
                            ...
                            render: function(data, type, row) {
                                // Giả sử trường 'description' chứa URL của hình ảnh
                                if (type === 'display') {
                                    return '<div style="width:100%; display: flex; justify-content: center;"><img src="../uploads/img/' + data + '" alt="" style="width: 40%;"></div>';
                                }
                                return data;
                            }
                        },
                ...
                // console.log(category_table.columns());
            }
        }
    ...
    $(document).on('submit', 'form#category_add_form', function(e) {
        ...
        // Read file in jquery then send to Ajax
        // Get the file separate
        ...
        $.ajax({
            ...
            // Set processData and contentType to false to stop jquery auto generated code
            processData: false,
            contentType: false,
            data: formData,
    ...
    $(document).on('click', 'button.edit_category_button', function() {
        $('div.category_modal').load($(this).data('href'), function() {
            ...
            $('form#category_edit_form').submit(function(e) {
                ...
                var data = form.serializeArray();
                // Read file in jquery then send to Ajax
                var file = $('#updateImg').prop('files')[0];
                var formData = new FormData();
                $.each(data, function(index, field) {
                    formData.append(field.name, field.value);
                });
